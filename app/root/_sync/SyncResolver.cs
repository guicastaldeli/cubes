...
public class SyncResolver {
    ...
    private object MergeValue(object? existing, object? incoming, Type type) {
        ...
        if(type.IsGenericType && type.GetGenericTypeDefinition() == typeof(List<>)) return MergeList(existing, incoming);
    ...
    public object? Resolve(object existing, object incoming, ConflictResolution resolution, long timestamp) {
        switch(resolution) {
            ...
            case ConflictResolution.LAST_WRITE_WINS: return LasyWriteWins(existing, incoming, timestamp);
            case ConflictResolution.MERGE_WITH_ARBITER: return MergeWithArbiyer(existing, incoming);
