...
class MeshRenderer : DataEntry {
    ...
    private bool useBufferReallocation = true;
    private BufferUsageHint bufferUsage = BufferUsageHint.DynamicDraw;
     ...
     *
     */
    // Upload Instance Data Reallocated
    private void uploadInstanceDataReallocated() {
        if(instanceCount == 0) return;

        GL.BindVertexArray(vao);

        float[] posData = new float[instanceCount * 3];
        for(int i = 0; i < instanceCount; i++) {
            posData[i*3+0] = cachedInstancePositions[i].X;
            posData[i*3+1] = cachedInstancePositions[i].X;
            posData[i*3+2] = cachedInstancePositions[i].X;
        }

        GL.BindBuffer(BufferTarget.ArrayBuffer, instanceVbo);
        GL.BufferData(BufferTarget.ArrayBuffer, posData.Length * sizeof(float), IntPtr.Zero, bufferUsage);
        GL.BufferSubData(BufferTarget.ArrayBuffer, IntPtr.Zero, posData.Length * sizeof(float), posData);
        GL.BufferSubData(BufferTarget.ArrayBuffer, IntPtr.Zero, posData.Length * sizeof(float), posData);
        GL.VertexAttribPointer(4, 3, VertexAttribPointerType.Float, false, 0, 0);
        GL.EnableVertexAttribArray(4);
        GL.VertexAttribDivisor(4, 1);

        if(cachedInstanceColors.Count == instanceCount) {
            float[] colorData = new float[instanceCount * 4];
            for(int i = 0; i < instanceCount; i++) {
                var c = cachedInstanceColors[i];
                colorData[i*4+0] = c[0];
                colorData[i*4+1] = c[1];
                colorData[i*4+2] = c[2];
                colorData[i*4+3] = c.Length > 3 ? c[3] : 1.0f;
            }

            GL.BindBuffer(BufferTarget.ArrayBuffer, instanceColorVbo);
            GL.BufferData(BufferTarget.ArrayBuffer, colorData.Length * sizeof(float), IntPtr.Zero, bufferUsage);
            GL.BufferSubData(BufferTarget.ArrayBuffer, IntPtr.Zero, colorData.Length * sizeof(float), colorData);
            GL.VertexAttribPointer(5, 4, VertexAttribPointerType.Float, false, 0, 0);
            GL.EnableVertexAttribArray(5);
            GL.VertexAttribDivisor(5, 1);
        }

        if(cachedInstanceRotations.Count == instanceCount) {
            float[] rotationData = new float[instanceCount * 4];
            for(int i = 0; i < instanceCount; i++) {
            ...
            GL.BindBuffer(BufferTarget.ArrayBuffer, instanceRotationVbo);
            GL.BufferData(BufferTarget.ArrayBuffer, rotationData.Length * sizeof(float), IntPtr.Zero, bufferUsage);
            GL.BufferSubData(BufferTarget.ArrayBuffer, IntPtr.Zero, rotationData.Length * sizeof(float), rotationData);
            GL.VertexAttribPointer(6, 4, VertexAttribPointerType.Float, false, 0, 0);
            GL.EnableVertexAttribArray(6);
            GL.VertexAttribDivisor(6, 1);
        }

        if(cacheInstanceTexIds.Count == instanceCount) {
            int[] texData = cacheInstanceTexIds.ToArray();

            GL.BindBuffer(BufferTarget.ArrayBuffer, instanceTexIdVbo);
            GL.BufferData(BufferTarget.ArrayBuffer, texData.Length * sizeof(int), IntPtr.Zero, bufferUsage);
            GL.BufferSubData(BufferTarget.ArrayBuffer, IntPtr.Zero, texData.Length * sizeof(int), texData);
            GL.VertexAttribIPointer(7, 1, VertexAttribIntegerType.Int, 0, 0);
            GL.EnableVertexAttribArray(7);
            GL.VertexAttribDivisor(7, 1);
        }

        if(cachedInstanceScales.Count == instanceCount) {
            float[] scaleData = cachedInstanceScales.ToArray();

            GL.BindBuffer(BufferTarget.ArrayBuffer, instanceScaleVbo);
            GL.BufferData(BufferTarget.ArrayBuffer, scaleData.Length * sizeof(float), IntPtr.Zero, bufferUsage);
            GL.BufferSubData(BufferTarget.ArrayBuffer, IntPtr.Zero, scaleData.Length * sizeof(float), scaleData);
            GL.VertexAttribPointer(8, 1, VertexAttribPointerType.Float, false, 0, 0);
            GL.EnableVertexAttribArray(8);
            GL.VertexAttribDivisor(8, 1);
        }
    ...
    private void uploadInstanceData() {
        ...
        GL.BindBuffer(BufferTarget.ArrayBuffer, instanceVbo);
        GL.BufferData(BufferTarget.ArrayBuffer, posData.Length * sizeof(float), posData, bufferUsage);
        GL.VertexAttribPointer(4, 3, VertexAttribPointerType.Float, false, 0, 0);
        GL.EnableVertexAttribArray(4);
        GL.VertexAttribDivisor(4, 1);
        ...
        if(cachedInstanceColors.Count == instanceCount) {
            ...
            GL.BindBuffer(BufferTarget.ArrayBuffer, instanceColorVbo);
            GL.BufferData(BufferTarget.ArrayBuffer, colorData.Length * sizeof(float), colorData, bufferUsage);
            GL.VertexAttribPointer(5, 4, VertexAttribPointerType.Float, false, 0, 0);
            GL.EnableVertexAttribArray(5);
            GL.VertexAttribDivisor(5, 1);
        }
        ...
        if(cachedInstanceRotations.Count == instanceCount) {
            ...
            GL.BindBuffer(BufferTarget.ArrayBuffer, instanceRotationVbo);
            GL.BufferData(BufferTarget.ArrayBuffer, rotationData.Length * sizeof(float), rotationData, bufferUsage);
            GL.VertexAttribPointer(6, 4, VertexAttribPointerType.Float, false, 0, 0);
            GL.EnableVertexAttribArray(6);
            GL.VertexAttribDivisor(6, 1);
        }
        ...
        if(cacheInstanceTexIds.Count == instanceCount) {
            ...
            GL.BindBuffer(BufferTarget.ArrayBuffer, instanceTexIdVbo);
            GL.BufferData(BufferTarget.ArrayBuffer, texData.Length * sizeof(int), texData, bufferUsage);
            GL.VertexAttribIPointer(7, 1, VertexAttribIntegerType.Int, 0, 0);
            GL.EnableVertexAttribArray(7);
            GL.VertexAttribDivisor(7, 1);
        }
        ...
        if(cachedInstanceScales.Count == instanceCount) {
            ...
            GL.BindBuffer(BufferTarget.ArrayBuffer, instanceScaleVbo);
            GL.BufferData(BufferTarget.ArrayBuffer, scaleData.Length * sizeof(float), scaleData, bufferUsage);
            GL.VertexAttribPointer(8, 1, VertexAttribPointerType.Float, false, 0, 0);
            GL.EnableVertexAttribArray(8);
            GL.VertexAttribDivisor(8, 1);
        }
